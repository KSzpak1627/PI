...
<div id="container" class="container">
		...
		<div class="kont col-xs-12">
				...
				<div class="title col-xs-12 no-padding">
		            <h3 class="text-center">Obozy językowe - oferta 2018</h3></div>
			...
			if (isset($_GET['id'])) 
				$sqlCommand="SELECT * FROM obozy WHERE IDobo='".mysqli_real_escape_string($myConnection, $_GET['id'])."'";			
			else		
				$sqlCommand="SELECT * FROM obozy";
			$guery=mysqli_query($myConnection, "set names 'utf8'");
			$query=mysqli_query($myConnection, $sqlCommand);        
			while($row = mysqli_fetch_array($query)){ ?>
					<div class='post-head'><?php echo $row['nazwaobo'];?></div>
			...
			$sqlCommand="SELECT * FROM zapisyobo WHERE IDobo = '".mysqli_real_escape_string($myConnection, $_GET['id'])."'";
			$query=mysqli_query($myConnection, $sqlCommand);        
			while($row = mysqli_fetch_array($query)){ 
				$buttons = '<a href="toggleUczK.php?iduser='.$row['IDuser'].'&idobo='.$row['IDobo'].'" class="btn btn-xs btn-danger">Wypisz</a>';
			?>
